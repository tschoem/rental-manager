...
export async function downloadAndStoreImage(imageUrl: string, folder: string = 'owner-images'): Promise<string> {
  try {
    ...
    console.log(`[DOWNLOAD] Uploading image to ${folder}/${filename} (size: ${buffer.length} bytes)`);
    const url = await uploadFile(buffer, filename, folder);
    console.log(`[DOWNLOAD] Successfully uploaded image. URL: ${url}`);

    // Return the public URL
    return url;
  } catch (error) {
    console.error(`[DOWNLOAD] Error downloading/storing image ${imageUrl}:`, error);
    console.error(`[DOWNLOAD] Error details:`, error instanceof Error ? error.message : String(error));
    // Re-throw the error instead of silently returning original URL
    // This way the import process knows the image failed
    throw new Error(`Failed to download and store image: ${error instanceof Error ? error.message : String(error)}`);
  }
}
